...
1. Check whether playwright-cli is available:
...
- 默认情况总是使用插件模式打开现有的浏览器，除非用户明确要求。
- 使用插件模式打开指定页面时，先执行 `playwright-cli attach --extension` chrome 建立扩展连接，再执行 `playwright-cli goto <url>` 进入目标页面。
- 相关本地文件包含图或者其他文件请保存在项目的 .playwright-cli 目录
